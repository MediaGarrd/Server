...
public abstract class AbstractLocalCopyRunner implements Runner {
    ...
    @Override
    public String getRunnerStartTime() {
        return startTime;
    }

    @Override
    public String getRunnerEndTime() {
        return endTime;
    }
    ...
    @Override
    public String getRunnerException() {
        return errorMessage;
    }
    ...
    public void run(Path outputDirectory) {
        ...
        try {
            ...
            errorMessage = "Unable to create output directory for " + serviceName;
            status = "failed";
        ...
        if (succeeded == specs.size()) {
            status = "completed";
        } else if (succeeded < specs.size() && succeeded > 0) {
            status = "partial";
        } else {
            status = "failed";
        }
    ...
    private void copySpec(CopySpec spec, Path serviceDirectory) {
        if (spec.sourcePath() == null || spec.sourcePath().isBlank()) {
            errorMessage = serviceName + " runner missing source path for " + spec.targetName();
            status = "failed";
        ...
        if (!Files.exists(source)) {
            errorMessage = serviceName + " source path not found: " + source;
            status = "failed";
